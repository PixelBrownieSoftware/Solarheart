...
namespace Solarheart
{
    ...
    */
    ...
    public struct s_shape
    {
        ...
        {
            foreach (Vector2 axis in e_solarHeart.axis)
            {
    ...
    public class s_object : s_generic
    {
        ...
        */
        ...
        public bool CheckPoint<T>(Vector2 point) where T : s_object
        {
            ...
            for (int i = 0; i < e_solarHeart.objects.Count; i++)
            {
                T obje = e_solarHeart.objects[i].GetComponent<T>();
                if (obje == null)
                    continue;
        ...
        public T CheckPointEnt<T>(Vector2 point) where T : s_object
        {
            ...
            for (int i = 0; i < e_solarHeart.objects.Count; i++)
            {
                if (e_solarHeart.objects[i] == null)
                    continue;
                T obje = e_solarHeart.objects[i].GetComponent<T>();
                if (obje == null)
                    continue;
    ...
    public class s_spriterend
    {
        ...
        public void SetSprite(string name, int num) {
            texture = e_solarHeart.textures[name].Item1;
            rect = e_solarHeart.textures[name].Item2[num];
        }
        ...
        public void SetSprite(string name, s_anim.s_frame frame)
        {
            switch (frame.type) {
                case s_anim.ANIM_TYPE.SPRITE:
                    texture = e_solarHeart.textures[name].Item1;
                    rect = e_solarHeart.textures[name].Item2[frame.pos];
                    sprEff = frame.fx;
                    break;
            }
        }
                ...
                if (currentANIMLENG > 0)
                {
                    if (animTimer <= 0)
                    {
                        if(currentFrame.type == s_anim.ANIM_TYPE.SOUND)
                            e_solarHeart.game.PlaySound(currentFrame.pos);
                        ...
                        animTimer -= e_solarHeart.deltaTime;
                }
                else
                {
                    currentFrame = currentAnimation.animations[currentFrameNumber];
    ...
    public class e_solarHeart : Game
    {
        public s_camera camera;
        public static e_solarHeart game;
        ...
        */
        ...
        public void AddFont(Vector2 fontsize, Vector2 fontglyph, string fontTexture)
        {
            List<char> characters = new List<char>();
            ...
            font = new s_font(fontsize, fontglyph, characters, textures[fontTexture].Item1);
        }
        public void AddFont(Vector2 fontsize, Vector2 fontglyph)
        {
            List<char> characters = new List<char>();
